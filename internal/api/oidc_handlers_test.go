...
func newTestHandlerWithOIDC(t *testing.T, store Store, oidc *auth.OIDCProvider) http.Handler {
	...
	strict := NewStrictHandlerWithOptions(
		NewServer("test", store, auth.SecureNever, oidc, NewLoginRateLimiter(), NewVerifyRateLimiter()),
...
func TestFindOrCreateOidcUser_CreatesShadowFromPreferredUsername(t *testing.T) {
	t.Parallel()
	m := newMemStore()
	s := NewServer("test", m, auth.SecureNever, nil, NewLoginRateLimiter(), NewVerifyRateLimiter()) // no provider needed for this helper
...
func TestFindOrCreateOidcUser_FallsBackToEmailLocalPart(t *testing.T) {
	t.Parallel()
	m := newMemStore()
	s := NewServer("test", m, auth.SecureNever, nil, NewLoginRateLimiter(), NewVerifyRateLimiter())
	u, err := s.findOrCreateOidcUser(context.Background(), &auth.OIDCClaims{
		Issuer: "https://idp.example.com",
		Sub:    "xyz",
