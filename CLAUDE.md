...
- `internal/store/` — PostgreSQL implementation of `api.Store` using `pgx/v5`. Cursor-paginated list, merge-patch updates, embedded `goose` migrations. FK chain: `clusters` → `namespaces` / `nodes` → `pods`, all `ON DELETE CASCADE`.
- `internal/collector/` — Kubernetes polling collector. Each tick fetches the API server version (refreshing the cluster record), lists all nodes (upserting by `(cluster_id, name)`), lists all namespaces (upserting by `(cluster_id, name)`), and lists all pods cluster-wide (upserting by `(namespace_id, name)` after resolving each pod's K8s namespace name against the id map built during namespace ingestion). When `ARGOS_COLLECTOR_RECONCILE=true` (default), rows that disappeared from the live listing are deleted so the CMDB mirrors the cluster — required for ANSSI cartography fidelity. Pods are reconciled per-namespace, so an emptied namespace has its stored pods cleared too. Reconciliation only runs after a successful list, so a transient Kubernetes error never wipes the store. Disabled by default; enable with `ARGOS_COLLECTOR_ENABLED=true` and `ARGOS_CLUSTER_NAME=...`.
