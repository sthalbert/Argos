...
longue-vue was built first as a single trusted-zone service: longue-vue holds the CMDB (cluster inventory, audit log, encrypted cloud-account secrets, sessions, tokens), serves the admin UI to humans, and accepts inventory pushes from collectors over the same REST API.

ADR-0009 introduced **`longue-vue-collector`** — a push-mode Kubernetes collector that runs *inside* a remote cluster (typically air-gapped, customer-owned, or on the other side of a tightly-controlled network boundary) and pushes inventory snapshots to longue-vue over HTTPS using a bearer PAT (ADR-0007). This works as long as longue-vue is reachable from wherever the collector runs.

In high-security deployments aligned with **ANSSI SecNumCloud** (the qualification framework longue-vue targets — ADR-0001), longue-vue cannot be reachable from the internet. The data it holds is too sensitive:
...
A direct-internet-exposed longue-vue is incompatible with the SNC posture customers expect. But **collectors must still be able to push from outside the trusted zone** — that is the entire point of ADR-0009. The collectors run wherever the customer's clusters live, and "wherever" frequently means an environment that can reach the public internet but not directly into the operator's trusted zone.

The standard answer to this shape of problem is a **DMZ ingest barrier**: a hardened component, deployed in a perimeter network, that accepts inbound writes from the internet, performs a strict allowlist + auth check, and forwards approved traffic into the trusted zone over a controlled inbound port. longue-vue never sees the public internet directly.

This ADR also carries one small but important refactor on longue-vue's side. `longue-vue-collector` today calls **one read endpoint** at startup (`GET /v1/clusters?name=…`) before deciding whether to POST or PATCH the cluster. A strict-write-only DMZ ingest path requires that read to disappear; the cleanest way to do that is to make `POST /v1/clusters` idempotent on `name`. The collector then unconditionally POSTs and never asks for a list.

This ADR does **not** apply to `longue-vue-vm-collector` (ADR-0015): that binary is operator-deployed in the DMZ itself (it needs internet egress to talk to cloud-provider APIs anyway) and reaches longue-vue directly across the same DMZ → trusted-zone hop the gateway uses. The vm-collector's narrow `vm-collector` token scope and per-account binding remain the boundary for that traffic.

This ADR decides the deployment topology, the trust model between gateway and longue-vue, the cert-source story, the endpoint allowlist, the token-verification cache, the cluster-bootstrap refactor, the configuration surface, the observability story, and the testing strategy for the new component.

## Decision

**Ship `longue-vue-ingest-gw`, a new stateless reverse-proxy binary deployed in the DMZ behind Envoy/WAF. It exposes a hardcoded write-only allowlist of 18 routes, verifies every bearer PAT against longue-vue via a new `POST /v1/auth/verify` endpoint with a 60 s in-memory cache, and forwards approved requests over mTLS to a new mTLS-only ingest listener on longue-vue. longue-vue's existing public listener is unchanged. The gateway is never an auth authority — longue-vue re-validates every forwarded token with full argon2id. longue-vue's `POST /v1/clusters` becomes idempotent on `name` so the collector no longer needs any read endpoint.**
...
```
  ...
  │ longue-vue-collector   │ ──────────► │  Envoy   │ ───────────► │ longue-vue-ingest-gw  │ ───────────► │  longue-vue  │
  ...
  Trusted-zone admins / UI / OIDC / MCP / cluster reads → longue-vue :8080 (existing public listener, unchanged)
```

The DMZ deploys **only** `longue-vue-ingest-gw`. It has:
...
- Outbound egress allowed *only* to longue-vue's ingest listener (NetworkPolicy enforced).
...
longue-vue grows **exactly one new listener** (`:8443`, mTLS-only) and **one new endpoint** (`POST /v1/auth/verify`). Its existing public listener (`:8080`), serving the UI, admins, OIDC, MCP, EOL, impact, audit, settings, and trusted-zone collectors, is unchanged in every respect. When the new listener's env vars are unset, longue-vue behaves exactly as today and the gateway is opt-in per deployment.

### 2. Endpoint allowlist (the security boundary)

The gateway compiles a hardcoded `(method, path-pattern)` table at build time. Anything not on this table returns `404 Not Found` (not `403` — the gateway does not reveal which routes exist on longue-vue):
...
- `GET /readyz` — ready when (a) the cert files are present and not within 1 hour of expiry **and** (b) the gateway can complete an mTLS handshake against longue-vue's `/healthz`
- `GET /metrics` — Prometheus, on the separate `:9090` listener bound to the pod IP

Path-matching rules:

- `{uuid}` segments validated with the same regex longue-vue uses; a malformed UUID returns `400` at the gateway without making any upstream call.
...
- Any `X-Longue-Vue-Verified-*` header on the inbound request is **stripped at ingress** so a collector cannot inject a "trusted caller" identity downstream.

The allowlist is the security boundary: a security review can read it as a single literal table and answer "what can reach longue-vue from the internet" without needing to understand the gateway's code paths.

**Read endpoints, admin endpoints, audit endpoints, settings endpoints, OIDC endpoints, MCP endpoints, EOL endpoints, impact endpoints, session endpoints, token-management endpoints, cloud-account credential endpoints, and VM-collector endpoints are all unreachable from the DMZ.** They live only on longue-vue's `:8080` listener, which is not exposed to the gateway's network or to the internet. Defense in depth: the gateway's allowlist already excludes them, and the ingest listener (§3) physically does not register them.

### 3. longue-vue ingest listener

A second `http.Server` started by `cmd/longue-vue/main.go` when `LONGUE_VUE_INGEST_LISTEN_ADDR` is set. Same process, separate listener, separate mux:
...
When `LONGUE_VUE_INGEST_LISTEN_ADDR` is empty, none of this listener's machinery starts and longue-vue behaves identically to today. Existing deployments are unaffected.
...
**The gateway code only knows about two paths**: `--tls-cert-file` and `--tls-key-file`, defaulting to `/etc/longue-vue-ingest-gw/tls/tls.crt` and `/etc/longue-vue-ingest-gw/tls/tls.key`. An `fsnotify` watcher on the directory invalidates an `atomic.Pointer[tls.Certificate]` used by `tls.Config.GetClientCertificate`. **Hot-reload, no pod restart, no matter who wrote the file.** A Prometheus counter increments on each successful reload; a separate counter increments on reload failures so renewal regressions are visible before the cert actually expires.

longue-vue's CA trust is orthogonal: a `--client-ca-file` (or `LONGUE_VUE_INGEST_LISTEN_CLIENT_CA_FILE`) points at whichever CA bundle signs the gateway's cert — Vault PKI intermediate, an internal CA, cert-manager's `ClusterIssuer`, etc. longue-vue does not care about the gateway's cert source.

**Recommended (but not required) Vault PKI configuration:**

- Mount: `pki_int`, role `longue-vue-ingest-gw` bound to namespace + ServiceAccount via the Kubernetes auth method.
...
- Vault Agent template writes `/etc/longue-vue-ingest-gw/tls/tls.crt` + `tls.key` atomically.
- Renewal failures: cert eventually expires → mTLS handshakes to longue-vue start failing → all writes 503 → `longue_vue_ingest_gw_cert_renewal_failures_total` and `longue_vue_ingest_gw_cert_not_after_seconds` Prometheus alerts page the operator. Collectors retry with backoff; no data loss provided longue-vue recovers within the collector's retry window.

longue-vue-side cert validation (defense in depth on top of "signed by `--client-ca-file`"):

1. **CN allowlist** (`LONGUE_VUE_INGEST_LISTEN_CLIENT_CN_ALLOW`, optional) — comma-separated allowed Subject CNs. When set, e.g. `longue-vue-ingest-gw`, blocks any other cert the same CA might be issuing.
2. **Cert expiry** — Go's stdlib enforces this; longue-vue re-checks in a `VerifyPeerCertificate` callback so the failure mode lands as a structured `longue_vue_ingest_listener_client_cert_failures_total{reason="expired"}` increment instead of an opaque handshake reset.
3. **No SAN-IP / SAN-DNS check** — the gateway dials longue-vue, never the reverse; longue-vue does not care what name the gateway thinks it has.

### 5. Token verification — `POST /v1/auth/verify` and the gateway cache

#### The new endpoint

A minimal, internal-only endpoint that reuses longue-vue's existing `auth.VerifyBearerToken` (same argon2id check the existing middleware does, no new auth path):
...
Invalid tokens return `401` with an RFC 7807 problem doc (`{"type":"…/invalid_token"}`) and no detail body. Auth on this endpoint is mTLS-only — there is no Authorization header on the verify call itself; the client-cert identifies the caller as the gateway. Any non-mTLS caller is rejected at the TLS handshake by the listener's `RequireAndVerifyClientCert` setting; the verify endpoint is not reachable on longue-vue's public `:8080` listener at all. Rate-limited at longue-vue to 100 req/s per source IP as a backstop if the gateway's cache is bypassed by a buggy build.
...
| Positive TTL | **60 s** | Revocation lag = 60 s worst case; longue-vue revoke + 60 s = fully effective at gateway |
...
| Forced eviction | `Cache-Control: no-store` on the verify response | Lets longue-vue push same-second revocation when needed |
| Concurrency | One in-flight verify per `sha256(token)`; siblings wait | Stops a thundering herd on token rotation or cold start |

The cache is *not* a substitute for longue-vue's per-request auth. Every proxied write is *also* argon2id-checked at longue-vue. The cache only saves the gateway from forwarding garbage. The gateway is never an auth authority: longue-vue re-validates everything, the cache exists purely to short-circuit invalid tokens before they cross the firewall and to drop verify-call cardinality on longue-vue.

Failure semantics:

| Situation | Gateway behavior |
|---|---|
| Cache miss → longue-vue `401` | Cache negative result 10 s, return `401` to collector. |
| Cache miss → longue-vue unreachable | Return `503` to collector. **Do not** fail-open. **Do not** cache. |
...
| Token expired between hit and write | longue-vue is the truth. Forward its `401`. |

### 6. longue-vue refactor — `POST /v1/clusters` is idempotent on `name`

Today, `POST /v1/clusters` returns `409 Conflict` on duplicate `name`. The collector handles this with a GET-then-decide pattern, calling `GET /v1/clusters?name=<name>&limit=1` first. That GET is the only read in `longue-vue-collector` and the only thing blocking strict-write-only.
...
- longue-vue handler change: ~30 lines in `internal/api/server.go` plus a duplicate-key catch in `internal/store/clusters.go`.
...
Both gateway and longue-vue run TLS listeners with identical hardening:
...
tls.Config{
    ...
    ClientAuth:             tls.RequireAndVerifyClientCert,  // longue-vue ingest listener only
    ClientCAs:              <PEM bundle>,                     // longue-vue ingest listener only
}
```
...
Connection pooling (gateway → longue-vue):
...
| Header | Treatment |
|---|---|
| `Authorization: Bearer …` | **Forwarded as-is.** longue-vue re-validates with full argon2id. |
| `X-Forwarded-For` (from Envoy) | Forwarded; longue-vue's audit log records this as source IP. |
| `X-Forwarded-Proto`, `X-Forwarded-Host` | Forwarded. |
| `X-Longue-Vue-Verified-*` (any) | **Stripped on ingress.** Collector cannot inject a "trusted" header. |
| `X-Real-IP` | Stripped, replaced with the connecting peer (Envoy). |
| `Host` | Rewritten to longue-vue's expected hostname. |
| Hop-by-hop (RFC 7230) | Stripped. |
| Everything else | Forwarded unchanged. |

### 8. Behavior when longue-vue is unreachable

Synchronous proxy. No buffering. No queue. No disk spool. On longue-vue unreachable (5xx, timeout, mTLS handshake failure):
...
- `longue-vue-collector`'s existing exponential-backoff retry handles recovery — the gateway sees the next attempt as a fresh request.
...
| `LONGUE_VUE_INGEST_GW_UPSTREAM_URL` | yes | — | e.g. `https://longue-vue-ingest.longue-vue.svc.cluster.local:8443` |
...
| `LONGUE_VUE_INGEST_GW_UPSTREAM_CA_FILE` | yes | — | CA bundle to verify longue-vue's cert |
| `LONGUE_VUE_INGEST_GW_CLIENT_CERT_FILE` | yes | `/etc/longue-vue-ingest-gw/tls/tls.crt` | mTLS client cert (vault/secret/file all converge) |
| `LONGUE_VUE_INGEST_GW_CLIENT_KEY_FILE` | yes | `/etc/longue-vue-ingest-gw/tls/tls.key` | mTLS client key |
...
longue-vue env vars (all `LONGUE_VUE_INGEST_LISTEN_*`):
...
Two charts ship in this repo. The DMZ deploys *only* `longue-vue-ingest-gw` — never the umbrella `longue-vue` chart — keeping the DMZ blast radius small and the trust boundary clean.

```
charts/
├── longue-vue/                       # existing — bumped appVersion; gains conditional ingest-listener bits
│   └── templates/
│       └── longue-vue-ingest-listener.yaml
└── longue-vue-ingest-gw/             # new chart
    ├── Chart.yaml
    ├── values.yaml
    ├── README.md
    └── templates/
        ...
        ├── networkpolicy.yaml       # egress only to longue-vue ingest port + Vault (vault mode only)
...
```yaml
image:
  repository: ghcr.io/longue-vue/longue-vue-ingest-gw
...
upstream:
  url: ""               # required: https://longue-vue-ingest.<longue-vue-ns>.svc.cluster.local:8443
...
networkPolicy:
  enabled: true
  longue-vueSelector:
    matchLabels: { app.kubernetes.io/name: longue-vue }
  longue-vueNamespace: longue-vue
  longue-vueIngestPort: 8443
  vaultEgress:
    enabled: false
...
- Default NetworkPolicy: egress allowed only to `longue-vueSelector` on `longue-vueIngestPort`, plus Vault CIDRs if vault mode is enabled. **No egress to internet, no egress to anything else in-cluster.**
- 2 replicas + `PodDisruptionBudget{minAvailable: 1}` so a node drain or rolling update never takes the gateway fully down.

Container image: distroless static, single static binary (~15 MiB), built from `Dockerfile.ingest-gw` (two-stage, no Node, no UI). Built by a new GH Actions job mirroring the existing `docker-build` job for longue-vue.
...
longue-vue ingest-listener metrics (extending existing):
...
**Never logged**: full token, full request body, full `Authorization` header, response body. `token_prefix` is the same 8-char hex that lives in `tokens.token_prefix` — enough to correlate with longue-vue's audit log, useless for replay. `envoy_request_id` is propagated end-to-end so a single ID traces an incident through Envoy → gateway → longue-vue's audit log.

Audit: longue-vue is the durable system of record. Every gateway-allowed request lands in `audit_events`. Every gateway-denied request is observable as a metric counter. The gateway itself does **not** keep an audit log — adding a second persistent log in the DMZ would create a second redaction surface and a second backup target without buying any audit fidelity.
...
- longue-vue is no longer reachable from the internet in DMZ deployments. Compromise of the gateway grants only the ability to forward requests longue-vue would also have allowed — no read access, no admin access, no token forging.
...
- longue-vue's existing trust model is unchanged. Every forwarded token is still argon2id-checked at longue-vue. The gateway is a filter, not an auth authority.
...
- A 60 s cache TTL means token revocation is not instantaneous through the gateway. Operators who need same-second revoke can issue `Cache-Control: no-store` from longue-vue or shorten the TTL. The default trades 60 s of revocation lag for a ~60× drop in verify-call cardinality on longue-vue.
- The two-listener split on longue-vue (`:8080` public + `:8443` ingest) doubles the TLS-cert ops surface for operators who enable ingest. Mitigated by Helm templating of the ingest-listener bits.
...
- The gateway is mTLS-required to longue-vue. Operators must wire a CA and a client cert one way or another. Three modes mitigate the ergonomic burden but the requirement itself is non-negotiable.
- The gateway does not buffer. longue-vue outage = collector backoff loop, not gateway-side replay. This is the correct trade for a stateless DMZ component but means a long longue-vue outage produces collector-side retry storms that need their own rate limiting (existing in `longue-vue-collector`).

## Alternatives Considered

### Pull-mode / store-and-forward via a queue in the DMZ

Considered: gateway accepts inbound writes from collectors and pushes them to a queue (NATS/Redis/Kafka) co-located in the DMZ; longue-vue polls the queue from the trusted zone. **Zero inbound from DMZ to trusted zone** — strictest possible network posture.
...
Considered: rather than ship a Go binary, configure an off-the-shelf proxy with allowlist rules, rate limits, and a Lua/WASM module to call longue-vue's verify endpoint and cache results.
...
### Reuse longue-vue as the gateway via a `--mode=gateway` flag
...
**Rejected**: one image to maintain, but two very different runtime postures sharing the same binary. Risk: "gateway mode accidentally exposes admin route because someone forgot to gate it" — exactly the failure mode you most want to avoid in a DMZ component. A separate binary with its own surface area makes "what code runs in the DMZ" answerable by reading `cmd/longue-vue-ingest-gw/main.go`.

### Switch PATs to JWTs so the gateway validates locally

Considered: longue-vue issues short-lived JWTs signed with a private key; gateway validates locally with the public key (JWKS). No verify call to longue-vue. Gateway works even during a longue-vue outage.

**Rejected for this ADR**: requires changing the auth model (collectors need refresh logic, longue-vue needs signing-key + JWKS endpoint, revocation becomes a denylist problem instead of a "delete the row" problem). The verify-cache approach gets most of the operational benefit (~60× cardinality drop on longue-vue) without inventing a second token format. JWTs can be layered on later if a future requirement is "the gateway must keep authenticating during a longue-vue outage" — until that requirement exists, the simpler model wins.

### Identity handoff via `X-Longue-Vue-Verified-Caller-Id` header (gateway as auth authority)

Considered: after a successful verify, gateway strips `Authorization`, injects `X-Longue-Vue-Verified-Caller-Id: <uuid>` + `X-Longue-Vue-Verified-Scope: write`, and longue-vue trusts the header (relying on mTLS for trust). Saves one argon2id check per request on longue-vue.

**Rejected**: the gateway becomes an auth authority. If anyone ever loosens mTLS or adds a header-based bypass code path on longue-vue, that header becomes a `sudo` primitive. The current design — forward the original token, let longue-vue re-validate — has a strictly weaker compromise outcome: a fully malicious gateway can only forward requests longue-vue would also have allowed.

### Strict write-only without the cluster-lookup refactor

Considered: keep `longue-vue-collector`'s startup `GET /v1/clusters?name=`, document the gateway as "writes plus this one bootstrap read".
...
### Spool buffer in the DMZ for longue-vue-down replay

Considered: 5-minute, 100 MiB on-disk spool that the gateway drains when longue-vue recovers, hiding brief outages from collectors.
...
1. **longue-vue refactor** (smallest, lands first): make `POST /v1/clusters` idempotent on `name`, drop the GET from `longue-vue-collector`. Triggers the OpenAPI validation test from the feature-workflow skill. No new binary, no new chart — just a regression-safe API change that benefits every deployment.
2. **longue-vue ingest listener + verify endpoint**: add `internal/api/ingest_mux.go`, `POST /v1/auth/verify`, the new env vars, the `audit_events.source` extension, and the body scrubber for the `token` field. Listener is gated on `LONGUE_VUE_INGEST_LISTEN_ADDR`; existing deployments unaffected. Tests: per §test-strategy.
3. **Gateway binary**: `cmd/longue-vue-ingest-gw/`, `internal/ingestgw/{allowlist,cache,verify_client,proxy,tls_reload,metrics}`. Distroless image via `Dockerfile.ingest-gw`. Unit tests exhaust the allowlist negatives.
4. **Helm chart `longue-vue-ingest-gw`**: three TLS modes (vault/secret/file), NetworkPolicy, PDB, optional ServiceMonitor + PrometheusRule. README walks through Vault-PKI setup, cert-manager setup, and manual-cert setup.
5. **Integration tests**: build-tagged `integration`, run against a real Postgres + real longue-vue + real gateway with a self-signed CA.
...
- **Unit (longue-vue)**: `POST /v1/auth/verify` (mTLS-only, audit redaction, rate limit), `POST /v1/clusters` idempotency, ingest listener route registration + CN allowlist.
- **Integration**: happy path, mid-stream revoke (cached "valid" doesn't override longue-vue's `401`), longue-vue restart returns `503` not buffer, bad-CA cert is rejected with metric increment, cert hot-reload mid-test, body cap at 11 MiB.
...
- **JWT-style ingest tickets** (alternative §D): if a future requirement is "the gateway must keep authenticating during a longue-vue outage," longue-vue can grow a "mint signed ingest ticket" endpoint and the gateway gains a JWKS-validation path. Reversible under the current design.
- **Per-collector rate limiting at the gateway**: today the gateway delegates rate limiting to Envoy/WAF + longue-vue. If a customer needs collector-tier rate limiting that survives an Envoy reconfiguration, a token-bucketed limiter keyed on `token_prefix` is a small addition.
- **OpenTelemetry tracing**: not in this ADR. Pick up across longue-vue in a future cross-cutting ADR.
- **Multi-region longue-vue / DR**: out of scope. The gateway addresses one upstream URL; HA is the ingress controller's problem above the gateway.
...
- RFC 7807 — Problem Details for HTTP APIs (the error-shape longue-vue already returns)
